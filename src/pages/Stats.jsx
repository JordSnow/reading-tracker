...
function computeStats(books) {
    ...
    const booksWithDates = books.filter(b => b.started_date && b.completed_date)
    const avgPagesPerDay = booksWithDates.length
        ? Math.round(booksWithDates.reduce((sum, b) => {
            const days = Math.max(1, (new Date(b.completed_date) - new Date(b.started_date)) / (1000 * 60 * 60 * 24))
...
function MonthlyBreakdown({ books, year }) {
    return (
        <div className="glass rounded-2xl p-5">
            <p className="text-white/50 text-xs font-medium uppercase tracking-wider mb-4">Monthly Breakdown {year}</p>
            <div className="space-y-3">
                {Array.from({ length: 12 }, (_, i) => {
                    ...
                    const monthPages = monthBooks.reduce((sum, b) => sum + (b.page_count || 0), 0)
                    const monthName = new Date(year, i, 1).toLocaleDateString('en-GB', { month: 'short' })
                        ...
                        <div key={month} className="flex items-center gap-3">
                            ...
                            <div className="flex-1 h-2 rounded-full overflow-hidden"
                                ...
                                <div className="h-full rounded-full transition-all"
                                     style={{
                                         width: `${Math.min(100, (monthBooks.length / Math.max(...Array.from({ length: 12 }, (_, j) => {
                                             return books.filter(b => b.completed_date && new Date(b.completed_date).getMonth() === j).length
                                         }))) * 100)}%`,
                                         background: '#E8682A'
                                     }} />
                            </div>
...
function StatsGrid({ stats }) {
    return (
        <div className="space-y-3">
            ...
            {stats.longest && (
                <StatCard label="Longest book" value={stats.longest.title}
                          sub={`${stats.longest.page_count} pages`} />
            )}
            {stats.shortest && (
                <StatCard label="Shortest book" value={stats.shortest.title}
                          sub={`${stats.shortest.page_count} pages`} />
            )}
            <StatCard label="Avg pages per book" value={stats.avgPages} />
            {stats.avgPagesPerDay && (
                <StatCard label="Avg pages per day" value={stats.avgPagesPerDay}
                          sub="from start to finish date" />
            )}
            {stats.avgDaysPerBook && (
                <StatCard label="Avg days per book" value={stats.avgDaysPerBook}
                          sub="from start to finish date" />
            )}
            {stats.topAuthor && (
                <StatCard label="Most read author" value={stats.topAuthor.name}
...
function Stats() {
    ...
    const filteredBooks = useMemo(() => {
        ...
        if (tab === 'month') return allBooks.filter(b => {
            if (!b.completed_date) return false
            const d = new Date(b.completed_date)
    ...
    const rollSuccessRate = rollResult.length > 0
        ? Math.round((rollResult.filter(b => b.status === 'Library').length / rollResult.length) * 100)
        : 0
        ...
        <div className="space-y-4">
                    ...
                    >
                        {Array.from({ length: 12 }, (_, i) => (
                            <option key={i} value={i} style={{ background: '#0d1117' }}>
                                {new Date(2024, i, 1).toLocaleDateString('en-GB', { month: 'long' })}
                            </option>
                        ))}
                    </select>
                </div>
            ...
            {!stats ? (
                <p className="text-white/30 text-sm text-center mt-16">
                    No books completed {tab === 'alltime' ? 'yet' : tab === 'month' ? 'this month' : `in ${year}`}.
                </p>
            ) : (
                <div className="space-y-3">
                    ...
                    {/* Roll success rate */}
                    <div className="glass rounded-2xl p-4">
                        <p className="text-white/40 text-xs uppercase tracking-wider mb-2">Random roll success rate</p>
                    ...
                    <div className="glass rounded-2xl p-5 text-center">
                        ...
                        <p className="text-white/30 text-xs mb-4">Your personalised reading summary</p>
