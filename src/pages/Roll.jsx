...
function BookCover({ book, size = 'md', dimmed = false }) {
    ...
    return (
        <div className={`${sizes[size]} rounded-2xl overflow-hidden shadow-2xl shrink-0 transition-all duration-300 ${dimmed ? 'opacity-40 scale-90' : 'opacity-100 scale-100'}`}>
            {book.cover_i ? (
...
function Roll() {
    ...
    function handleRoll() {
        ...
        function next() {
            ...
            if (tick < totalTicks) {
                ...
                // Pick final book
                const picked = eligibleBooks[Math.floor(Math.random() * eligibleBooks.length)]
                setFinalBook(picked)
                setPhase('result')
            }
        }
    ...
    if (hasTableBook) {
        return (
            <div className="text-center mt-16 space-y-4">
                ...
                <p className="text-white font-semibold">You're already reading something!</p>
                <p className="text-white/40 text-sm">Finish your current book before rolling a new one.</p>
    ...
    if (eligibleBooks.length === 0) {
        return (
            <div className="text-center mt-16 space-y-4">
                ...
                <p className="text-white/40 text-sm">Add some roll-eligible books to your shelf first.</p>
    ...
    const visible = phase === 'spinning' && carouselBooks.length > 0
        ? [-2, -1, 0, 1, 2].map(offset => {
            const idx = (centerIndex + offset + carouselBooks.length) % carouselBooks.length
    ...
    return (
        <div className="space-y-8">
            <div className="text-center">
                <h1 className="text-2xl font-bold text-white">Random Roll</h1>
                <p className="text-white/30 text-sm mt-1">{eligibleBooks.length} books eligible</p>
            </div>
            ...
            <div className="relative flex items-center justify-center overflow-hidden"
                ...
                {phase === 'idle' && (
                    <div className="text-center space-y-3">
                        ...
                        <p className="text-white/30 text-sm">Hit roll to pick your next book</p>
                    </div>
                )}
                ...
                {phase === 'result' && finalBook && (
                    <div className="flex flex-col items-center space-y-4">
                        <p className="text-white/40 text-xs uppercase tracking-wider">Your next read is...</p>
            ...
            {phase === 'result' && finalBook && (
                <div className="text-center">
                    ...
                    {finalBook.page_count && (
                        <p className="text-white/30 text-xs mt-1">{finalBook.page_count} pages</p>
                    )}
                </div>
            )}
